...
class CartItemService
{
  ...
  public function addToCart(array $data): CartItem
  {
    ...
    if ($variant->stock_quantity < $data['quantity']) {
      throw ValidationException::withMessages(['quantity' => ['الكمية غير متوفرة.']]);
    }
    ...
    if ($cartItem) {
      ...
      if ($variant->stock_quantity < $newQuantity) {
        throw ValidationException::withMessages(['quantity' => ['تجاوزت المخزون.']]);
      }
  ...
  public function updateQuantity(int $id, int $quantity): CartItem
  {
    ...
    if ($variant->stock_quantity < $quantity) {
      throw ValidationException::withMessages(['quantity' => ['المخزون غير كافٍ.']]);
    }
